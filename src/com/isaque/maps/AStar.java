...
public class AStar {
    ...
    public static List<Node> findPath(Maps map, Vector2i start, Vector2i end){
        ...
        while (openList.size() > 0){
            ...
            for (int i = 0; i < 9; i++){
                ...
                int xi = (1%3) - 1;
                int yi = (1/3) -1;
                Tile tile = Maps.getTile(x+ xi + ((y+yi) * Maps.TILE_SIZE));
                if (tile == null) continue;
                if (tile instanceof Wall) continue;
                if (i == 0){
                    Tile test = Maps.getTile(x+xi+1+((y+yi) * Maps.TILE_SIZE));
                    Tile test2 = Maps.getTile(x+xi+1+((y+yi) * Maps.TILE_SIZE));
                    if (test instanceof Wall || test2 instanceof Wall){
                        continue;
                    }
                } else if (i == 2){
                    Tile test = Maps.getTile(x+xi+1+((y+yi) * Maps.TILE_SIZE));
                    Tile test2 = Maps.getTile(x+xi+((y+yi) * Maps.TILE_SIZE));
                    if (test instanceof Wall || test2 instanceof Wall){
                        continue;
                    }
                } else if (i == 6){
                    Tile test = Maps.getTile(x+xi+((y+yi) * Maps.TILE_SIZE));
                    Tile test2 = Maps.getTile(x+xi+1+((y+yi) * Maps.TILE_SIZE));
                    if (test instanceof Wall || test2 instanceof Wall){
                        continue;
                    }
